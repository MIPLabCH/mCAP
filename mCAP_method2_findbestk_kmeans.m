...
n_rep=100;
n_iter=300;
number_of_folds=1; % for now run once 
...
%% 4.b Initialize parpool if indicated 
...
%% 4.c  Unchange parameters of mCAP
...
%%% Temporal clustering train
fprintf(fid,['___Kmeans test...\n']);



    
[idxb{it},Cmb{it},SUMDb{it},Db{it}] = kmeans(cell2mat(Xonb)',ik,'Options',options,'distance','correlation','replicates'...
...
fprintf(fid,['___Kmeans...\n']);
    ...
    [idx{it},Cm{it},SUMD{it},D{it}]=kmeans(cell2mat(Xon)',ik,'Options',options,'distance','correlation','replicates',n_rep,'empty','drop','maxiter',n_iter);
frames=cell2mat(Xon);

%%% obtaining caps improvement based on hungarian matching 
...
traincap=Cm{it};
testcap=Cmb{it};
discentroids=pdist2(traincap,testcap,'correlation');
idxcentroids=munkres(discentroids);

dis_of_match{it}=discentroids(idxcentroids); clear testcap traincap discentroids idxcentroids
mean_dis_of_match(it,1)=mean(dis_of_match{it});
...
mat_save.Cmb=Cmb;clear Cmb;
mat_save.Db=Db; clear Db;
mat_save.idxb=idxb; clear idxb;
mat_save.SUMDb=SUMDb; clear SUMDb;
...
mat_save.mean_dis_cap=mean_dis_cap; %clear mean_dis_cap;
...
mat_save.dis_of_match_alliter=dis_of_match; clear dis_of_match
mat_save.mean_of_dis_match_alliter=mean_dis_of_match;
new_dismatch_meantofollow=mean(mean_dis_of_match{end}); clear mean_dis_of_match

fprintf(fid,['__________________ saving variables...\n']);


%%% saving resuts fo ik 
out_dir_sub=fullfile(out_dir,['fold_',char(num2str(crosval))]);
if ~isdir(out_dir_sub);  mkdir(out_dir_sub); end


save_dir=fullfile(out_dir_sub,['TTmCAP_conv',char(num2str(isconverged)),'_K',char(string(ik))]);
mkdir(save_dir);
...
testsumd(pel,1)=mean_max_sumD;
matchmean(pel,1)=new_dismatch_meantofollow;
clear mat_save mean_max_sumD mean_dis mean_dis_cap mean_dis_cap_first
end

save(fullfile(out_dir_sub,'mean_of_testSUMD.mat'),'testsumd','matchmean','new_dismatch_meantofollow','order_rand');
